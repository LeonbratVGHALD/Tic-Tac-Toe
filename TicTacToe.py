...
class TicTacToeApp:
	...
	def __init__(self, root):
		...
		# Sett applikasjonsikon (prøver PNG først, så .ico)
		try:
			self.set_app_icon()
		except Exception:
			pass
	...
	def set_app_icon(self):
		...
		Tries PNG files in `Icon/PNG` using `iconphoto` (preferred, cross-platform).
		Falls back to `.ico` or `.icns` in `Icon/ICO` using `iconbitmap`.
		"""
		base_dir = os.path.dirname(__file__)
		icon_dir = os.path.join(base_dir, "Icon")
		png_dir = os.path.join(icon_dir, "PNG")
		ico_dir = os.path.join(icon_dir, "ICO")

		# Try PNGs first (iconphoto)
		try:
				...
				if png_files:
					def size_of(fname):
						try:
							...
							return 0
					best = max(png_files, key=size_of)
					png_path = os.path.join(png_dir, best)
		...
		# Fallback: try .ico or .icns via iconbitmap
		try:
			if os.path.isdir(ico_dir):
				candidates = ['TacTacToeIcon.ico', 'TacTacToeIcon.icns']
